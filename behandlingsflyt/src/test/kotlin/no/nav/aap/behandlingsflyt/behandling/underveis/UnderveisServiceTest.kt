...
class UnderveisServiceTest {
    ...
    fun `Persistert rettighetstype med kvotevilkår skal gi samme svar som utledet bortsett fra rett der det er avslag på kvote`() {
        ...
        vilkårsresultat2.leggTilHvisIkkeEksisterer(Vilkårtype.ORDINÆR_KVOTE)
            .setVilkårTidslinje(OrdinærKvoteVilkår.vurder(OrdinærKvoteFaktagrunnlag(kvotevurdering, fiktiveKvoter)))
        vilkårsresultat2.leggTilHvisIkkeEksisterer(Vilkårtype.SYKEPENGEERSTATNING_KVOTE)
            .setVilkårTidslinje(
    ...
    fun `oppfylt kvotevilkår bruker av kvoten hvis hverdag, ikke oppfylt er avslag på kvote, og ikke relevant bruker ikke av kvoten`() {
        ...
        vilkårsresultat.leggTilHvisIkkeEksisterer(Vilkårtype.ORDINÆR_KVOTE)
            .setVilkårTidslinje(OrdinærKvoteVilkår.vurder(OrdinærKvoteFaktagrunnlag(kvotevurdering, fiktiveKvoter)))
        vilkårsresultat.leggTilHvisIkkeEksisterer(Vilkårtype.SYKEPENGEERSTATNING_KVOTE)
            .setVilkårTidslinje(
                SykepengeerstatningKvoteVilkår.vurder(
                    SykepengeerstatningKvoteFaktagrunnlag(
                        kvotevurdering,
