...
class StudentV1FlytTest : AbstraktFlytOrkestratorTest(
    StudentStegV1FlytTestUnleash::class
) {
    ...
    fun `innvilge som student, revurdering ordinær`() {
        ...
        behandling = behandling.løsAvklaringsBehov(
            ...
            }.løsAndreStatligeYtelser().løsAvklaringsBehov(ForeslåVedtakLøsning()).fattVedtak().medKontekst {
        ...
        sak.opprettManuellRevurdering(
        ...
        }.løsBistand(sak.rettighetsperiode.fom).løsSykdomsvurderingBrev().bekreftVurderinger()
            ...
            .løsOppholdskrav(fom).løsSykestipend().medKontekst {
                ...
                Periode(
                    fom, fom.plussEtÅrMedHverdager(ÅrMedHverdager.FØRSTE_ÅR)
    ...
    fun `revurdering med tom løsningerForPerioder skal ikke kaste NoSuchElementException`() {
        ...
        sak.opprettManuellRevurdering(listOf(Vurderingsbehov.REVURDER_STUDENT)).løsAvklaringsBehov(
    ...
    fun `Periodisering - skal kunne avslutte student i samme behandling`() {
        ...
        behandling = behandling.løsAvklaringsBehov(
            ...
            val studentVilkår = vilkår.finnVilkår(Vilkårtype.STUDENT)
            studentVilkår.tidslinje().assertTidslinje(Segment(Periode(fom, 31 desember 2025)) { vurdering ->
                assertThat(vurdering.utfall).isEqualTo(Utfall.OPPFYLT)
            ...
            .løsRefusjonskrav().løsSykdomsvurderingBrev().bekreftVurderinger().kvalitetssikre().løsBeregningstidspunkt()
            .løsOppholdskrav(fom).løsSykestipend(listOf(sykestipendPeriode)).medKontekst {
                val vilkår = repositoryProvider.provide<VilkårsresultatRepository>().hent(this.behandling.id)
                val syksetipendVilkår = vilkår.finnVilkår(Vilkårtype.SAMORDNING_ANNEN_LOVGIVNING)
                assertThat(syksetipendVilkår.harPerioderMedIkkeOppfylt()).isTrue
            }.løsAndreStatligeYtelser().løsAvklaringsBehov(ForeslåVedtakLøsning()).fattVedtak().medKontekst {
...
object StudentStegV1FlytTestUnleash : FakeUnleashBaseWithDefaultDisabled(
    enabledFlags = listOf()
)
