...
class FasttrackMeldekortFlytTest :
    ...
    fun `Meldekortgrunnlag skal flettes inn i åpen behandling før UnderveisSteg`() {
        ...
        assertThat(
            åpenBehandling.vurderingsbehov().map { it.type }).hasSameElementsAs(listOf(Vurderingsbehov.MOTTATT_SØKNAD))
        ...
        dataSource.transaction { connection ->
            ...
            assertThat(
                meldekortbehandling.vurderingsbehov()
    ...
    fun `Åpen behandling skal tilbakeføres til UnderveisSteg etter fullført meldekortbehandling`() {
        ...
        åpenBehandling = dataSource.transaction { connection ->
            ...
            val avklaringsbehov =
                AvklaringsbehovRepositoryImpl(connection).hentAvklaringsbehovene(meldekortbehandling.id).alle()
                    .filter { it.erÅpent() }
