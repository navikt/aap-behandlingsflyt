...
class OpprettBehandlingFritakMeldepliktJobbUtfører(
    ...
    override fun utfør(input: JobbInput) {
        ...
        taSkriveLåsRepository.låsSak(sakId).use {
    ...
    private fun skalHaFritakForPassertMeldeperiode(sak: Sak):Boolean {
        ...
        val dato = LocalDate.now().minusDays(7)
        val sistePasserteMeldeperiode =
            meldeperiodeRepository.hentMeldeperioder(sisteIverksatteBehandling.id, Periode(dato, dato))
                .firstOrNull { it.inneholder(dato) } ?: return false

        return underveisPerioder.somTidslinje()
            .begrensetTil(sistePasserteMeldeperiode)
            .segmenter().any { (_, underveisperiode) ->
                underveisperiode.utfall == Utfall.OPPFYLT
