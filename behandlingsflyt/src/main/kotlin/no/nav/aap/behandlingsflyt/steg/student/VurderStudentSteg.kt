...
import no.nav.aap.student.StudentFaktagrunnlag
import no.nav.aap.student.skalVurdereStudent
                ...
                when (kontekst.vurderingType) {
                    VurderingType.FØRSTEGANGSBEHANDLING, VurderingType.MIGERING_FRA_ARENA-> {
                        tidligereVurderinger.muligMedRettTilAAP(kontekst, type()) &&
                                (studentGrunnlag.skalVurdereStudent() || Vurderingsbehov.REVURDER_STUDENT in kontekst.vurderingsbehovRelevanteForSteg) && !unleashGateway.isEnabled(
                            BehandlingsflytFeature.StudentV2)
                    }
                    VurderingType.REVURDERING -> {
                        tidligereVurderinger.muligMedRettTilAAP(kontekst, type()) &&
                                Vurderingsbehov.REVURDER_STUDENT in kontekst.vurderingsbehovRelevanteForSteg && !unleashGateway.isEnabled(
                            BehandlingsflytFeature.StudentV2)
                    }
