...
class SykdomsvilkårFraLansering(vilkårsresultat: Vilkårsresultat) : Vilkårsvurderer<SykdomsFaktagrunnlag> {
    ...
    private fun opprettVilkårsvurdering(
        ...
        } else if (sykdomVurdering?.erOppfyltForYrkesskade(grunnlag.kravDato) == true && yrkesskadeVurdering?.erÅrsakssammenheng == true) {
            utfall = Utfall.OPPFYLT
        ...
        } else if (sykepengeerstatningVilkår.isEmpty() && sykepengerVurdering?.harRettPå == true && sykdomVurdering?.erOppfyltSettBortIfraVissVarighet() == true) {
            utfall = Utfall.OPPFYLT
