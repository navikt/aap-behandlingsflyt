...
class MedlemskapLovvalgVurderingService {
    ...
    private fun oppgittUtenlandsOpphold(grunnlag: UtenlandsOppholdData?): TilhørighetVurdering {
        ...
        val jsonGrunnlag = DefaultJsonMapper.toJson(grunnlag) // TODO: Her må vi faktisk lande hva vi vil ha ut
        return TilhørighetVurdering(listOf(Kilde.SØKNAD), Indikasjon.UTENFOR_NORGE, "Opphold i utland", !grunnlag.harBoddINorgeSiste5År, jsonGrunnlag)
    }
    ...
    private fun manglerStatsborgerskapIEØS(grunnlag: PersonopplysningGrunnlag): TilhørighetVurdering {
        ...
        val manglerEØSLandDeprecated = grunnlag.brukerPersonopplysning.land !in enumValues<EØSLand>().map { it.name }

        val jsonGrunnlag = grunnlag.let { DefaultJsonMapper.toJson(it) } // TODO: Her må vi faktisk lande hva vi vil ha ut
        return TilhørighetVurdering(listOf(Kilde.PDL), Indikasjon.UTENFOR_NORGE, "Mangler statsborgerskap i EØS", manglerEØS && manglerEØSLandDeprecated, jsonGrunnlag)
    }
