...
class ArenaMeldeperiodesyklusInformasjonskrav(
    ...
    override fun erRelevant(
        ...
        return unleashGateway.isEnabled(BehandlingsflytFeature.SynkroniserArenaMeldeperiodesyklus)
                ...
                && meldeperiodeRepository.hentFørsteMeldeperiode(kontekst.behandlingId) == null
    }
    ...
    override fun oppdater(
        ...
        when (registerdata) {
            ...
            is HarSyklus -> {
                ...
                val fom = LocalDate.parse("$år-W$isoUke-1", DateTimeFormatter.ISO_WEEK_DATE)
                val tom = fom.plusDays(13)
                val meldeperiode = Periode(fom, tom)
                /* Tar ikke og justerer meldeperioden for å matche rettighetsperiode,
                 * det håndteres av [FastsettMeldeperiodeSteg]. */
                meldeperiodeRepository.lagreFørsteMeldeperiode(kontekst.behandlingId, meldeperiode)
