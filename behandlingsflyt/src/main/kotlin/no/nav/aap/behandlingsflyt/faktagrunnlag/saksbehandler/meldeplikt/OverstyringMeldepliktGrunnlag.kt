...
data class OverstyringMeldepliktGrunnlag(
    ...
    private fun OverstyringMeldepliktVurdering.tilTidslinje(): Tidslinje<OverstyringMeldepliktData> {
        return Tidslinje(
            perioder.map { it.tilTidslinjeSegment(vurdertAv, vurdertIBehandling, opprettetTid) }
        )
    }

    private fun OverstyringMeldepliktVurderingPeriode.tilTidslinjeSegment(vurdertAv: String, vurdertIBehandling: BehandlingReferanse, opprettetTid: LocalDateTime?): Segment<OverstyringMeldepliktData> {
        return Segment(
