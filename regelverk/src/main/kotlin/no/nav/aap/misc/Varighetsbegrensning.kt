...
fun <T, R> Tidslinje<T>.mapMedDatoTilDatoVarighet(
    ...
    for (vurdering in this.segmenter()) {
        if (harBegrensetVarighet(vurdering.verdi)) {
            ...
            if (okPeriode != null) {
                leggTil(okPeriode, Varighetsvurdering.VARIGHET_OK, vurdering.verdi)
            }

            for (overskredetPeriode in vurdering.periode.minus(datoTilDato)) {
                leggTil(overskredetPeriode, Varighetsvurdering.VARIGHET_OVERSKREDET, vurdering.verdi)
            }
        } else {
            leggTil(vurdering.periode, Varighetsvurdering.VARIGHET_OK, vurdering.verdi)
        }
    }
