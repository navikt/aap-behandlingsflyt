...
fun NormalOpenAPIRoute.formkravGrunnlagApi(
    ...
    route("api/klage/{referanse}/grunnlag/formkrav") {
        getGrunnlag<BehandlingReferanse, FormkravGrunnlagDto>(
            ...
            påkrevdRolle = Definisjon.VURDER_FORMKRAV.løsesAv
        ) { req ->
            val respons = dataSource.transaction(readOnly = true) { connection ->
                ...
                formkravRepository.hentHvisEksisterer(behandling.id)
                    ?.tilDto(kanSaksbehandle(), ansattInfoService)
                    ?: FormkravGrunnlagDto(harTilgangTilÅSaksbehandle = kanSaksbehandle())
            }
            respond(respons)
        }
    }
