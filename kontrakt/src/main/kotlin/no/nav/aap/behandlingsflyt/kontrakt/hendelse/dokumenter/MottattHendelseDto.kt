    ...
    init {
        when (type) {
            ...
            InnsendingType.FORELDREPENGE_VEDTAK_HENDELSE -> {
                require(referanse.type == InnsendingReferanse.Type.FORELDREPENGEVEDTAK_HENDELSE_ID)
                requireNotNull(melding) {"Melding fra foreldrepengevedtakhendelse kan ikke være null"}
                require(melding is ForeldrepengeVedtakV0)
            }
            ...
            InnsendingType.ARBEIDSUTPRØVING -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.BARNETILLEGG -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.ETABLERING -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.FENGSEL_VARETEKT -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.LÆRLING -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.MEDLEMSKAP -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.PARTSINNSYN -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.REFUSJONSKRAV -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.SLUTTAVTALE -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.STUDENTBESTEMMELSEN -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.YRKESSKADE -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.KARAKTERUTSKRIFTER_OG_CV -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.HELSEOPPLYSNINGER -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.ETTERSENDELSE_TIL_KLAGE -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.ETTERSENDELSE_TIL_FEILUTBETALING -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
            InnsendingType.TILTAKSRAPPORT -> {
                require(referanse.type == InnsendingReferanse.Type.JOURNALPOST)
                requireNotNull(melding)
                require(melding is AnnetRelevantDokument)
            }
